...
graph TD
    ...
    B -- "Valid" --> D{"Validate & Decrypt Code"}
    D -- "Invalid/Expired" --> E(["Return AccountInvalidCredentials"])
    D -- "Valid" --> F{"Find Account"}
    F -- "Not Found/Deleted" --> G(["Return AccountInvalidCredentials/Deactivated"])
    F -- "Found" --> H["Find Account Profile"]
    H --> I["Create Account Session"]
    I --> J["Generate JWT"]
    J --> K(["Return Token Response"])
```
