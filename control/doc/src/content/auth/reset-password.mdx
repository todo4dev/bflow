...
graph TD
    ...
    B -- "Valid" --> D{"Find Active Account"}
    D -- "Not Found/Not Active" --> E(["Return AccountNotFound"])
    D -- "Found" --> F{"Verify OTP"}
    F -- "Invalid/Mismatch" --> G(["Return AccountInvalidOTP"])
    F -- "Valid" --> H["Consume OTP (Delete from Cache)"]
    H --> I["Hash New Password"]
    I --> J["Update Account Credential"]
    J --> K["Publish AccountCredentialChanged Event (Async)"]
    K --> L(["Return Success"])
```
