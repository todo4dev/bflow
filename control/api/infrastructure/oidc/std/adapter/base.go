// infrastructure/oidc/std/adapter/base.go
package adapter
...
func (b *httpBase) GetAuthURL(state map[string]any, extraParams map[string]string) string {
	...
	q.Set("state", b.EncodeState(state))
...
func (b *httpBase) EncodeState(state map[string]any) string {
	data, _ := json.Marshal(state)
	return base64.RawURLEncoding.EncodeToString(data)
}

func (b *httpBase) DecodeState(base64State string) (map[string]any, error) {
	data, err := base64.RawURLEncoding.DecodeString(base64State)
	if err != nil {
		return nil, err
	}
