// application/auth/refresh_authorization_token/refresh_authorization_token.go
...
func (h *Handler) deleteAccountSession(key string) {
	go func() {
		...
		if err := h.cacheClient.Delete(ctx, key); err != nil {
			h.loggingLogger.Error(ctx, "Failed to delete expired session", err)
		}
	}()
}
...
func init() {
	data := Data{RefreshToken: "header.payload.signature"}
